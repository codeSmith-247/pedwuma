...
class Model extends Middleware
{
    ...
    public function __construct(string $host = 'localhost', string $username = 'root', string $password = '', string $database = 'peadwuma')
    {
        $this->host = $host;
    ...
    protected function groupWhere($values, $where = false, $operator = '')
    {
    ...
    protected function andWhere(...$where): object
    {
        return $this->condition('and', ...$where);
    }

    protected function rawCondition($sql, $parameters = [])
    {
    ...
    protected function paginate(int $per_page = 50): object
    {
    ...
    protected function execute(bool $result = true)
    {
        ...
        $this->query_string = " select $selects from {$this->query_table} {$this->query_join_string} {$this->query_conditions} {$this->query_offsets}";

        if (!$result) {
            $this->error($this->query_string);
        }

        $this->query_parameters = array_merge($this->subquery_parameters, $this->query_parameters);
